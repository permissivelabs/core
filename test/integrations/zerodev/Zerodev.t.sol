...
contract ZerodevTest is Test {
    ...
    function setUp() public {
        ...
        permissive = new ZerodevValidator(verifier);
    }
    ...
    function testSwap(uint256 operatorPrivateKey) public {
        ...
        deal(CONIC, address(kernel), 1 ether);
        vm.deal(address(kernel), 1 ether);
        vm.prank(address(kernel));
        entryPoint.depositTo{value: 1 ether}(address(kernel));
