...
import {SafePlugin} from "../../../src/integrations/safe/new-spec/SafePlugin.sol";
...
import {SafeProtocolManager} from "safe-core/SafeProtocolManager.sol";
import {SafeProtocolRegistry, Enum} from "safe-core/SafeProtocolRegistry.sol";
...
contract Safe4337Test is Test {
    ...
    SafeL2 safe;
    SafePlugin permissive;
    EntryPoint entryPoint;
    PermissionVerifier verifier;
    PermissionExecutor executor;
    PermissionRegistry registry;
    FeeManager feeManager;
    SafeProtocolManager safeManager;
    SafeProtocolRegistry safeRegistry;
    ...
    function setUp() public {
        ...
        safeRegistry = new SafeProtocolRegistry(address(this));
        safeManager = new SafeProtocolManager(
            address(this),
            address(safeRegistry)
        );
        permissive = new SafePlugin(
            address(entryPoint),
            address(verifier),
            address(safeManager),
            address(executor)
        );
        safeRegistry.addIntegration(
            address(permissive),
            Enum.IntegrationType.Plugin
        );
        vm.prank(address(safe));
        safeManager.enablePlugin(address(permissive), true);
        ...
        vm.prank(address(safe));
        safe.enableModule(address(safeManager));
    }
    ...
    function testSwap(uint256 operatorPrivateKey) public {
        ...
        console.log(
            safeManager.isPluginEnabled(address(safe), address(permissive)),
            address(safe),
            address(permissive)
        );
